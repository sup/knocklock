/* Detects patterns of knocks and triggers a motor to unlock
   ...
   Altered by Charles Lai to use a servo motor and slightly different behavior
...
const int yellowLED = 3;           // Status LED
...
const int enterTime = 100;         // seconds we wait before we lock the door
...
void triggerDoorUnlock(){
  ...
  // turn the servo and unlock the door
  myServo.write(0);
  ...
  // While the button is not pressed, keep the door unlocked
  while (digitalRead(programSwitch)==LOW) {
    
  }
  
  // wait a bit before locking the door
  delay (enterTime);                
